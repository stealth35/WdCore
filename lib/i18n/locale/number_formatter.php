 ...
 * @since 1.0
 */
class WdLocaleNumberFormatter extends WdObject
{
	private $locale;
	...
	public function format($value, $pattern=null, $currency=null)
	{
		...
		else if (($symbol = $this->locale->getCurrencySymbol($currency)) === null)
		{
			$symbol = $currency;
		}
	...
	{
		return $this->format($this->locale->conventions['numbers']['currencyFormats']['currencyFormatLength']['currencyFormat']['pattern'], $value, $currency);
	}
	...
	protected function format_number($value, $format)
	{
		...
		if(strlen($decimal)>0)
			$decimal=$this->locale->getNumberSymbol('decimal').$decimal;
